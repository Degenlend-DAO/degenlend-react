...
export const updateusdcBorrowRate = createAsyncThunk(
  "usdc/updateBorrowAPY",
  async () => {
    let borrowRate = 0;
...
export const approveUSDC = createAsyncThunk(
  "usdc/approve",
  async ({ amount, addressToApprove }: approveUSDCParams) => {
    try {
      const provider = new ethers.BrowserProvider(
        window.ethereum as unknown as Eip1193Provider,
      );
      const signer = await provider.getSigner();
      const signedUSDC = new ethers.Contract(
        address.testnetUSDC,
        erc20ABI,
        signer,
      );
...
export const repayUSDC = createAsyncThunk(
  "usdc/repay",
...
export const borrowUSDC = createAsyncThunk(
  "usdc/borrow",
  async (borrowAmount: number) => {
    const provider = new ethers.BrowserProvider(
      window.ethereum as unknown as Eip1193Provider,
