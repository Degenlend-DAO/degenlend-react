...
export const updateWSXBalance = createAsyncThunk(
  ...
  async () => {
    ...
    console.log(
      `wsx balance: ${balance}, raw balance: ${rawBalance}, gas balance: ${gasBalance}`,
    );
...
export const approveWSX = createAsyncThunk(
  "wSX/approve",
  async ({ amount, addressToApprove }: approveWSXParams) => {
    try {
      const provider = new ethers.BrowserProvider(
        window.ethereum as unknown as Eip1193Provider,
      );
      const signer = await provider.getSigner();
      const signedWSX = new ethers.Contract(
        address.testnetWSX,
        erc20ABI,
        signer,
      );
...
export const supplyWSX = createAsyncThunk(
  "wSX/supply",
  async (supplyAmount: number) => {
    const provider = new ethers.BrowserProvider(
      window.ethereum as unknown as Eip1193Provider,
    );
    const signer = await provider.getSigner();
...
export const withdrawWSX = createAsyncThunk(
  "wSX/withdraw",
  async (supplyAmount: number) => {
    const provider = new ethers.BrowserProvider(
      window.ethereum as unknown as Eip1193Provider,
