...
const Dashboard: React.FC = () => {
  ...
  const supplyBalance = useSelector(
    (state: RootState) => state.supplyBalance.netSupplyBalance,
  );
  const borrowBalance = useSelector(
    (state: RootState) => state.borrowBalance.netBorrowBalance,
  );
  const borrowLimit = useSelector(
    (state: RootState) => state.borrowLimit.borrowLimit,
  );
        ...
        <Col span={6} offset={3}>
          <Card bordered={false}>
            <Statistic
              ...
              prefix={
                <img width="20" height="20" src={sxtoken} alt="WSX Token"></img>
              }
              loading={false}
            />
          </Card>
